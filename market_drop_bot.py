...
def main() -> None:
    ...
    # Revisa la cuota de OddsPapi sin consumir cuota (/account es no medido).
    ...
    while True:
        try:
            if time.time() >= next_quota_check:
                try:
                    acct = api.account()
                    subs = acct.get("subscriptions") or [] if isinstance(acct, dict) else []
                    sub = next((x for x in subs if isinstance(x, dict) and x.get("is_active")), subs[0] if subs else {})
                    used = int(sub.get("request_count") or 0) if isinstance(sub, dict) else 0
                    limit = int(sub.get("request_limit") or 0) if isinstance(sub, dict) else 0
                    if last_quota_count >= 0 and used < last_quota_count:
                        quota_alerted.clear()
                    last_quota_count = used
                    for threshold in (180, 220, 240):
                        if used >= threshold and threshold not in quota_alerted:
                            tg.send(
                                f"⚠️ CUOTA ODDSPAPI\n\n"
                                f"Usadas: {used}/{limit or '?'}\n"
                                f"Alerta alcanzada: {threshold}\n\n"
                                "El bot sigue funcionando. Revisá la cuota antes de llegar al límite."
                            )
                            quota_alerted.add(threshold)
                    next_quota_check = time.time() + 21600
                except Exception as e:
                    log(f"No pude revisar cuota OddsPapi: {e}")
                    next_quota_check = time.time() + 3600
